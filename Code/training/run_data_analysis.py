...
from Code.training.mm_modeling import (BertForSequenceClassification, ResNetForSequenceClassification, VitForSequenceClassification,
                                     ...
                                     ResBertTFNForMMSequenceClassification, Bert2ResForMMSequenceClassification, Vit22BertForMMSequenceClassification,
                                     Vit2BertForMMSequenceClassification, VitBertTFNForMMSequenceClassification, Bert2VitForMMSequenceClassification,
                                     ResBertAttForMMSequenceClassification, VitBertAttForMMSequenceClassification, Faster22BertForMMSequenceClassification,
                                     Faster2BertForMMSequenceClassification, FasterBertAttForMMSequenceClassification, FasterBertForMMSequenceClassification,
                                     FasterBertTFNForMMSequenceClassification, FasterRCNNForSequenceClassification, Bert2FasterForMMSequenceClassification)
from Code.training.optimization import BertAdam
from Code.training.file_utils import PYTORCH_PRETRAINED_BERT_CACHE

import Code.resnet.resnet as resnet
from Code.resnet.resnet_utils import myResnet
...
def image_process(image_path, transform, encoder='resnet', num_box=36):
    if encoder == 'resnet' or encoder == 'vit':
        image = Image.open(image_path).convert('RGB')
        image = transform(image)
    elif encoder == 'faster':
        ann = json.load(open(image_path, 'r'))
        max_conf = base64.b64decode(bytes(ann['max_conf'], encoding='utf-8'))
        max_conf = torch.frombuffer(max_conf, dtype=torch.float32)
        pooled_feat = base64.b64decode(bytes(ann['feature'], encoding='utf-8'))
        pooled_feat = torch.frombuffer(
            pooled_feat, dtype=torch.float32).reshape(-1, 2048)
        keep_boxes = torch.argsort(max_conf, descending=True)[:num_box]

        #print(max_conf.shape, pooled_feat[keep_boxes].shape, flush=True)
        image = pooled_feat[keep_boxes]
...
class AbmsaProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "dev.tsv")), "dev")  # TODO
...
def convert_mm_examples_to_features(examples, label_list, max_seq_length, max_entity_length, tokenizer, crop_size, path_img, encoder='vit'):
    """Loads a data file into a list of `InputBatch`s."""
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if encoder == 'vit':
            added_input_mask = [1] * (len(input_ids)+197)
        elif encoder == 'resnet':
            added_input_mask = [1] * (len(input_ids)+49)
        elif encoder == 'faster':
            added_input_mask = [1] * (len(input_ids)+36)
        ...
        if encoder == 'vit' or encoder == 'resnet':
            image_path = os.path.join(path_img, image_name)
        elif encoder == 'faster':
            image_path = os.path.join(
                path_img, image_name.replace('.jpg', '.json'))

        if not os.path.exists(image_path):
            print(image_path)
        try:
            image = image_process(image_path, transform, encoder)
        except:
            count += 1
            #print(f'image {image_path} has problem!')
            image_path_fail = os.path.join(path_img, '17_06_4705.jpg')
            image = image_process(image_path_fail, transform, encoder)
...
def main():
    ...
    parser.add_argument("--img_model",
                        ...
                        type=str)
    parser.add_argument("--encoder", default='vit', type=str)
    ...
    parser.add_argument('--bertlayer', action='store_true',
                        help='whether to add another bert layer')
    parser.add_argument('--tfn', action='store_true',
    ...
    if args.task_name == "Twitter17":        # this refers to Twitter17 dataset
        if args.encoder == 'resnet' or args.encoder == 'vit':
            args.path_image = "/root/RethinkingTMSC/data/Twitter17/images/"
        elif args.encoder == 'faster':
            args.path_image = "/root/RethinkingTMSC/data/Twitter17/faster_features/"
    elif args.task_name == "Twitter15":  # this refers to Twitter15 dataset
        if args.encoder == 'resnet' or args.encoder == 'vit':
            args.path_image = "/root/RethinkingTMSC/data/Twitter15/images/"
        elif args.encoder == 'faster':
            args.path_image = "/root/RethinkingTMSC/data/Twitter15/faster_features/"
    else:
        print("The task name is not right!")

    processors = {
        "Twitter15": AbmsaProcessor,  
        "Twitter17": AbmsaProcessor      
    }
    ...
    if args.mm_model == 'ResBert':
        model = ResBertForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                   ...
                                                                   tfn=args.tfn)
    elif args.mm_model == 'ResNet':
        model = ResNetForSequenceClassification.from_pretrained(args.bert_model,
                                                                cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                    args.local_rank),
                                                                num_labels=num_labels,
                                                                )
    elif args.mm_model == 'FasterRCNN':
        model = FasterRCNNForSequenceClassification.from_pretrained(args.bert_model,
                                                                    cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                        args.local_rank),
                                                                    num_labels=num_labels,
                                                                    )
    elif args.mm_model == 'FasterBert':
        model = FasterBertForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                      cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                          args.local_rank),
                                                                      num_labels=num_labels,
                                                                      )
    elif args.mm_model == 'Faster2Bert':
        model = Faster2BertForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                       cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                           args.local_rank),
                                                                       num_labels=num_labels,
                                                                       )
    elif args.mm_model == 'FasterBertTFN':
        model = FasterBertTFNForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                         cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                             args.local_rank),
                                                                         num_labels=num_labels,
                                                                         )
    elif args.mm_model == 'Faster22Bert':
        model = Faster22BertForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                        cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                            args.local_rank),
                                                                        num_labels=num_labels,
                                                                        )
    elif args.mm_model == 'FasterBertAtt':
        model = FasterBertAttForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                         cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                             args.local_rank),
                                                                         num_labels=num_labels,
                                                                         )
    elif args.mm_model == 'Bert2Faster':
        model = Bert2FasterForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                       cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                           args.local_rank),
                                                                       num_labels=num_labels,
                                                                       )
    elif args.mm_model == 'Vit':
        model = VitForSequenceClassification.from_pretrained(args.bert_model,
                                                             cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                 args.local_rank),
                                                             num_labels=num_labels,
                                                             )
    elif args.mm_model == 'Bert':
        model = BertForSequenceClassification.from_pretrained(args.bert_model,
                                                              cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                  args.local_rank),
                                                              num_labels=num_labels,
                                                              )
    elif args.mm_model == 'VitBert':
        model = VitBertForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                   cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                       args.local_rank),
                                                                   num_labels=num_labels,
                                                                   bert_layer=args.bertlayer,
                                                                   tfn=args.tfn)
    elif args.mm_model == 'ResBertTFN':
        model = ResBertTFNForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                      cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                          args.local_rank),
                                                                      num_labels=num_labels,
                                                                      )
    elif args.mm_model == 'ResBertAtt':
        model = ResBertAttForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                      cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                          args.local_rank),
                                                                      num_labels=num_labels,
                                                                      )
    elif args.mm_model == 'VitBertTFN':
        model = VitBertTFNForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                      cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                          args.local_rank),
                                                                      num_labels=num_labels,
                                                                      )
    elif args.mm_model == 'VitBertAtt':
        model = VitBertAttForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                      cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                          args.local_rank),
                                                                      num_labels=num_labels,
                                                                      )
    elif args.mm_model == 'Res2Bert':
        model = Res2BertForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                    cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                        args.local_rank),
                                                                    num_labels=num_labels,
                                                                    )
    elif args.mm_model == 'Bert2Res':
        model = Bert2ResForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                    cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                        args.local_rank),
                                                                    num_labels=num_labels,
                                                                    )
    elif args.mm_model == 'Res22Bert':
        model = Res22BertForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                     cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                         args.local_rank),
                                                                     num_labels=num_labels,
                                                                     )
    elif args.mm_model == 'Vit2Bert':
        model = Vit2BertForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                    cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                        args.local_rank),
                                                                    num_labels=num_labels,
                                                                    )
    elif args.mm_model == 'Bert2Vit':
        model = Bert2VitForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                    cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                        args.local_rank),
                                                                    num_labels=num_labels,
                                                                    )
    elif args.mm_model == 'Vit22Bert':
        model = Vit22BertForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                     cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                         args.local_rank),
                                                                     num_labels=num_labels,
                                                                     )
    ...
    if args.encoder == 'resnet':
        ...
        net.load_state_dict(torch.load(
            os.path.join(args.resnet_root, 'resnet152.pth')))
        encoder = myResnet(net, args.fine_tune_cnn, device)
    elif args.encoder == 'vit':
        encoder = AutoModel.from_pretrained(
            args.img_model, add_pooling_layer=False)
    elif args.encoder == 'faster':
        encoder = None
    ...
    if args.do_train:
        train_features = convert_mm_examples_to_features(
            train_examples, label_list, args.max_seq_length, args.max_entity_length, tokenizer, args.crop_size, args.path_image, args.encoder)
        ...
        eval_features = convert_mm_examples_to_features(
            eval_examples, label_list, args.max_seq_length, args.max_entity_length, tokenizer, args.crop_size, args.path_image, args.encoder)
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                with torch.no_grad():
                    if args.encoder == 'resnet':
                        ...
                        img_att = encoder(img_feats).last_hidden_state
                    elif args.encoder == 'faster':
                        img_att = img_feats
            ...
            if eval_accuracy >= max_acc:
                ...
                if encoder is not None:
                    encoder_to_save = encoder.module if hasattr(
                        encoder, 'module') else encoder  # Only save the model it-self
                if args.do_train:
                    logger.info(f'!!!best epochs {train_idx+1}!!!')
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        fout_p = open(os.path.join(args.output_dir, "pred_" +
                      args.test_file.replace('.tsv', '.txt')), 'w')
        fout_t = open(os.path.join(args.output_dir, "true_" +
                      args.test_file.replace('.tsv', '.txt')), 'w')
        ...
        output_eval_file = os.path.join(
            args.output_dir, "eval_results_"+args.test_file.replace('.tsv', '.txt'))
